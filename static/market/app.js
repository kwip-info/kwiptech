...
(() => {
  ...
  async function jobs_poc(){
    ...
    root.innerHTML=`<p class="tag">${esc(result.notice)}</p><div class="stats"><section class="panel"><p class="eyebrow">Sample records</p><div class="stat">${number(summary.records)}</div></section><section class="panel"><p class="eyebrow">Companies</p><div class="stat">${number(summary.companies)}</div></section><section class="panel"><p class="eyebrow">With stated pay</p><div class="stat">${number(summary.with_pay)}</div></section></div>
      <section class="panel"><h2>Standardized postings</h2><p>US-eligible remote roles and external NYC agency postings. This is not a representative US market sample. Agency postings do not imply remote work or individual eligibility. Missing listings do not indicate closure.</p><label for="jobs-filter">Find a title or company in this sample</label><input id="jobs-filter" type="search" placeholder="Title or company" maxlength="100"><div class="inline"><div><label for="jobs-page-size">Records per page</label><select id="jobs-page-size"><option value="5">5</option><option value="10">10</option><option value="20">20</option></select></div></div><div id="jobs-rows"></div><nav class="inline" aria-label="Job pages"><button id="jobs-prev" type="button">Previous</button><span id="jobs-page-status" role="status"></span><button id="jobs-next" type="button">Next</button></nav><section id="job-details" class="panel" tabindex="-1" aria-label="Selected job details" hidden></section></section>
      <section class="panel"><h2>Sources & update plan</h2><p>Refreshes are manual and limited to once per 24 hours per source. Each source has its own request budget.</p><p>Source discovery review: ${esc(result.discovery.next_review_on)} · every ${esc(result.discovery.cadence_days)} days. New sources require review before collection.</p>
      ${table(['Collection source','State','Next permitted collection'],(result.collection_status||[]).map(s=>[esc(s.name),esc(s.status.replaceAll('_',' ')),s.next_allowed_at?esc(new Date(s.next_allowed_at).toLocaleString()):'Not collected or awaiting review']))}
      ...
      <section class="panel"><h2>Collection receipts</h2>${result.runs.length?table(['Attempt / source','Result','Accepted / examined','Upstream requests'],result.runs.map(r=>[esc(new Date(r.started_at).toLocaleString())+'<br>'+esc(r.source||''),esc(r.status)+(r.summary.error?'<br>'+esc(r.summary.error):''),`${number(r.summary.accepted||0)} / ${number(r.summary.examined||0)}`,`${number(r.summary.data_requests||0)} data · ${number(r.summary.robots_requests||0)} robots`])):'<p>No collection attempts yet.</p>'}</section>`;
    ...
    function showDetails(row,button){
      ...
      panel.innerHTML=`<div class="section-heading"><h2>${esc(d.title)}</h2><button id="job-details-close" type="button">Close details</button></div><p>${esc(d.company)} · ${esc(d.employment_type||'Employment type not supplied')}</p>
        <dl class="job-facts"><dt>Published by source</dt><dd>${esc((d.source_posting_date||dateText(d.published_at)))}</dd><dt>Source expiry</dt><dd>${esc((d.source_closing_text||dateText(d.expires_at)))}</dd><dt>Observed by KWIP</dt><dd>${esc(dateText(row.observed_at))}</dd><dt>Location eligibility</dt><dd>${esc(d.location_restrictions)}</dd><dt>Time zone restrictions</dt><dd>${esc(d.timezone_restrictions||'Not supplied')}</dd>${d.work_location?`<dt>Work location</dt><dd>${esc(d.work_location)}</dd>`:''}${d.residency_requirement?`<dt>Residency requirements</dt><dd>${esc(d.residency_requirement)}</dd>`:''}${d.source_updated_date?`<dt>Source updated date</dt><dd>${esc(d.source_updated_date)}</dd>`:''}<dt>Stated compensation</dt><dd>${salary(d)}</dd></dl>
        ...
        <p><a href="${esc(safeURL(row.source_url))}" rel="noopener noreferrer">View source record on ${esc(row.source.name)} ↗</a></p><p class="quiet">${esc(row.source.attribution)}</p><p class="quiet">Source expiry does not prove the role was filled. Dates reflect the source and our observation, not a verified hiring outcome.</p>`;
    ...
    function render(){
      ...
      $('#jobs-rows').innerHTML=visible.length?table(['Role & company','Eligibility','Stated pay','Dates & source'],visible.map((r,i)=>[
        ...
        `<small>Published ${esc((r.data.source_posting_date||dateText(r.data.published_at)))}<br>Observed ${esc(dateText(r.observed_at))}</small><br><a href="${esc(safeURL(r.source_url))}" rel="noopener noreferrer">${esc(r.source.name)} source record ↗</a>`])):
